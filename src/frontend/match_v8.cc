...
*/
{
  ...
  sort(prev_matches.begin(), prev_matches.end(), [](match a, match b) {
...
vector<int> get_source_notes( vector<midi_event> notes, int start_time, int start_index, int max_notes, int max_time )
/*
Args:
...
vector<match> find_matches_at_timestamp( int i, vector<midi_event> notes, bool disp, vector<match> prev_matches = {})
/*
    Args:
        i:
        notes:
...
void program_body( const string& midiPath, const string& outputPath, const string& paramPath )
{
  ...
  cout << "Percentage speedups: " << (100.0 * (double)num_speedups * (double)SKIP) / (double)(END - START) << endl;
